...
public class LabelTextInterceptorTest {
    ...
    private ExposedResourceBundleMessageSource stubExposedResourceBundleMessageSource = mock(ExposedResourceBundleMessageSource.class);
    private LabelTextInterceptor interceptor = new LabelTextInterceptor(stubExposedResourceBundleMessageSource);
    private HttpServletRequest stubRequest = mock(HttpServletRequest.class);
    private HttpServletResponse stubResponse = mock(HttpServletResponse.class);
    private ModelAndView mockModelAndView = mock(ModelAndView.class);
    private Object dummyHandler = new Object();
    ...
    public void common_label_should_be_added() throws Exception {
        given_messages_with_key_and_message(new SimpleEntry<>("label.common", "Common"));
    ...
    private void given_view_name(String viewName) {
        when(mockModelAndView.getViewName()).thenReturn(viewName);
    }

    private void given_message_keys(List<String> keys) {
        when(stubExposedResourceBundleMessageSource.getKeys(eq(LABEL_TEXT_SHORT_NAME), any(Locale.class))).thenReturn(new HashSet<>(keys));
    }
    ...
    private void given_messages_with_key_and_message(SimpleEntry<String, String>... messageEntries) {
        ...
        Stream.of(messageEntries).forEach(this::given_message_key_with_message);
    }

    private List<String> keysOf(SimpleEntry<String, String>[] messageEntries) {
        return Stream.of(messageEntries).map(SimpleEntry::getKey).collect(Collectors.toList());
    }
}
